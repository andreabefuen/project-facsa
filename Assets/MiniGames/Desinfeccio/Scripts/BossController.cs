...
public class BossController : MonoBehaviour {
    ...
    void Start()
	{
        ...
        score = 2500;
		timeShoot = 300 / Manager.MaxBullets;
		timeShoot1 = 500 / Manager.MaxBullets;
		timeShoot2 = 600 / Manager.MaxBullets;
        changeAction = true;
        Manager.BossLive += 100;
        intermedio = false;
        Manager.EnemiesInScene += 1;
        destroyed = false;
    }
    void Update() {

        if (Manager.BossLive <= 50 && changeAction)
        {
            if (!intermedio)
            {
    ...
    void Shoot(int cannon)
	{
		if (cannon == 0) {
			...
			timeShoot = UnityEngine.Random.Range (300 / Manager.MaxBullets, 600 / Manager.MaxBullets);
		}else if (cannon == 1) {
			Vector3 pos = new Vector3 (transform.position.x + 1.7f, transform.position.y - 1.2f, 0);
			GameObject newBullet = Instantiate (bullet, pos, bullet.transform.rotation);
			newBullet.transform.LookAt (target.position);
			newBullet.transform.Rotate (new Vector3 (0, -90, 90), Space.Self);//correcting the original rotation
			timeShoot1 = UnityEngine.Random.Range (400 / Manager.MaxBullets, 700 / Manager.MaxBullets);
		}else if(cannon == 2) {
			Vector3 pos = new Vector3 (transform.position.x + -1.7f, transform.position.y - 1.2f, 0);
			GameObject newBullet = Instantiate (bullet, pos, bullet.transform.rotation);
			newBullet.transform.LookAt (target.position);
			newBullet.transform.Rotate (new Vector3 (0, -90, 90), Space.Self);//correcting the original rotation
			timeShoot2 = UnityEngine.Random.Range (400 / Manager.MaxBullets, 700 / Manager.MaxBullets);
		}
        audioSource.Play();
    }
